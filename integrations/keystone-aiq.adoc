...
NOTE: O consumo de capacidade nas assinaturas do Keystone é exibido em TiBs nos painéis e relatórios, e é arredondado para duas casas decimais. Se o uso for inferior a 0,01 TiB, o valor será exibido como 0 ou Sem uso. Os dados nessas telas são exibidos em horário UTC (fuso horário do servidor). Ao inserir uma data para consulta, ela é automaticamente considerada em horário UTC. Para saber mais sobre métricas de uso, consulte link:../concepts/metrics.html["Medição de métricas"]. Para obter informações sobre as diferentes capacidades usadas no Keystone, consulte link:../concepts/supported-storage-capacity.html["Tipos de capacidade de storage em Keystone"].
