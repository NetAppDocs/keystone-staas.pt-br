...
Você pode visualizar a projeção de uso da capacidade para os próximos 120 dias a partir da data atual através da opção *Previsão*, disponível na visualização *Histórico de consumo* da aba *Tendência de consumo* no Console. A opção está ativada por padrão.

Por padrão, o gráfico exibe os últimos 30 dias de consumo histórico e uma projeção de 120 dias a partir da data atual no mesmo gráfico. Até 30 pontos de dados são plotados no total (20 para dados históricos e 10 para dados de previsão), com uma linha vertical separando os dois segmentos.

image:forecast-1.png["captura de tela mostrando dados de previsão"]

Se você selecionar um intervalo de datas que não termine na data atual, a previsão será desativada automaticamente. Para reativá-la, uma caixa de diálogo de confirmação informa que os dados de previsão só estão disponíveis quando o período selecionado termina na data atual. A confirmação redefine a visualização para o intervalo padrão dos últimos 30 dias de dados históricos mais os próximos 120 dias de previsão.

Caso o consumo projetado exceda a capacidade máxima, um indicador de alerta aparecerá no gráfico, recomendando uma atualização de capacidade.
...
Você pode visualizar o uso acumulado de dados em rajadas pelo Console ou Digital Advisor. Se você selecionou *Accrued burst* no menu suspenso *View* na guia *Consumption trend* no Console, ou a opção *Invoiced Accrued Burst* na guia *Consumption Trend* do Digital Advisor, você pode ver o uso acumulado de dados em rajadas mensalmente ou trimestralmente, dependendo do período de faturamento selecionado. Esses dados estão disponíveis para os últimos 12 meses faturados, e você pode consultar pelo intervalo de datas de até 30 meses anteriores.

No Console, além do uso de burst faturado, você também pode visualizar o uso de burst não faturado (acumulado, mas ainda não cobrado) e o uso de burst provisório (acumulado no período de faturamento atual e ainda não faturado).
...
Você também pode baixar os dados em formato Excel, incluindo o status do burst (faturado, não faturado ou provisório) e o período de faturamento.
